...
const outsideFunnel = fs.existsSync(path.join(root, '.selfforge/outside-funnel-review.jsonl')) ? lines(path.join(root, '.selfforge/outside-funnel-review.jsonl')).length : 0;
const report = { command_files: commandFiles.length, top_level_commands: topLevelCount, visible_commands: readmeCommands.length, mechanism_mentions: (read(path.join(root, 'README.md')).match(/mechanism/gi) || []).length, negative_controls: negativeFiles.length, incidents: incidents, hashed_incidents: hashed, outcomes: outcomes, closures: closures, outside_funnel_findings: outsideFunnel };
