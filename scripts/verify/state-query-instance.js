...
const instance = payload.ref && payload.ref.params && payload.ref.params.instance || null;
let bridgeStatement = declared.statement;
if (instance) {
  let spec;
  try { spec = JSON.parse(bridgeStatement); } catch (error) { fail('pinned statement is not JSON: ' + error.message); }
  Object.assign(spec, instance);
  bridgeStatement = JSON.stringify(spec);
}
const bridgePayload = {
  statement: bridgeStatement,
  statement_sha256: sha256Text(bridgeStatement),
