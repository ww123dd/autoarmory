...
const clean = JSON.parse(result.out); must(result.code === 0 && clean.exit_code === 0 && clean.input.count === 0 && clean.output.passed === true && clean.counterexample.observed === 0, 'count=0 must pass');
result = run(['--count', '1', '--json']);
const dirty = JSON.parse(result.out); must(result.code === 1 && dirty.exit_code === 1 && dirty.input.count === 1 && dirty.output.passed === false && dirty.counterexample.observed === 1, 'count>0 must fail');
result = run([]);
