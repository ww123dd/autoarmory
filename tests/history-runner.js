...
fs.writeFileSync(path.join(pending,'change-win-service.json'),JSON.stringify({schema_version:'autoarmory/pending-change/v1',change_id:'change-win-service',session_id:'s1',turn_id:'t1',source_message_id:'m1',changed_files:[],commands:[],check_status:'check_gap',signals:['risk_signal','check_gap'],verifier_id:'windows-service-state',expected_transition:'STATE->RUNNING',claim_instance:{service:'win-service'}}),'utf8');
...
fs.writeFileSync(path.join(pending,'change-win-service-new-claim.json'),JSON.stringify({schema_version:'autoarmory/pending-change/v1',change_id:'change-win-service',session_id:'s1',turn_id:'t2',source_message_id:'m2',changed_files:[],commands:[],check_status:'check_gap',signals:['risk_signal','check_gap'],verifier_id:'windows-service-state',expected_transition:'STATE->STOPPED',claim_instance:{service:'win-service'}}),'utf8');
...
fs.writeFileSync(path.join(pending,'change-untrusted-provenance.json'),JSON.stringify({schema_version:'autoarmory/pending-change/v1',change_id:'change-untrusted-provenance',session_id:'s2b',signals:['check_gap'],expected_transition:'TEST->PASS',expected_provenance:'agent_inferred',verifier_candidate:{kind:'project_test',ref:'node tests/signal-recall.js'},commands:['node tests/signal-recall.js'],mechanical_binding:{kind:'project_test',command_sha256:sha256('node tests/signal-recall.js'),cwd:repo}}),'utf8');
r=spawnSync(process.execPath,[script,'--drain','--state',state,'--repo',repo,'--json'],{encoding:'utf8'});
must(r.status===0 && JSON.parse(r.stdout).unverifiable_count===1,'untrusted expected provenance must be unverifiable');
const untrusted=JSON.parse(fs.readFileSync(path.join(state,'reuse-records','change-untrusted-provenance.json'),'utf8'));
must(untrusted.status==='unverifiable'&&/expected_provenance_untrusted/.test(untrusted.reason),'untrusted expected provenance must fail closed');
const repoHead=String(spawnSync('git',['rev-parse','HEAD'],{cwd:repo,encoding:'utf8'}).stdout||'').trim();
fs.writeFileSync(path.join(pending,'change-project-test.json'),JSON.stringify({schema_version:'autoarmory/pending-change/v1',change_id:'change-project-test',session_id:'s3',signals:['check_gap'],expected_transition:'TEST->PASS',expected_provenance:'baseline_manifest',claim_instance:{command:'node tests/signal-recall.js'},expected_value:0,verifier_candidate:{kind:'project_test',ref:'node tests/signal-recall.js'},commands:['node tests/signal-recall.js'],mechanical_binding:{kind:'project_test',command_sha256:sha256('node tests/signal-recall.js'),cwd:repo,repo_head:repoHead}}),'utf8');
r=spawnSync(process.execPath,[script,'--drain','--state',state,'--repo',repo,'--json'],{encoding:'utf8'});
...
must(projectReuse.expected_provenance==='baseline_manifest'&&projectReuse.claim_instance&&projectReuse.claim_instance.command==='node tests/signal-recall.js','mechanical reuse record must preserve expected provenance and claim instance');
