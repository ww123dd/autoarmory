...
const usage = [{ source_decision_id: 'usage-1', run_id: 'usage-1', cost_usd: 0.5, tokens: { total: 50 }, outcome: { status: 'success' }, skill_invoked: true, observed_at: '2026-01-01T00:06:00.000Z' }, { source_decision_id: 'usage-2', run_id: 'usage-2', cost_usd: 0.2, tokens: { total: 20 }, outcome: { status: 'failure' }, skill_invoked: false, observed_at: '2026-01-01T00:07:00.000Z' }];
const built = history.buildReplayRecords({ mechanism_runs: runs, routing_decisions: decisions, outcomes: outcomes, routing_actuals: [], transitions: [], usage_records: usage }, { failure_signatures: { 'same-signature': 1 }, previous_results: [] });
must(built.records.length === 8, 'every eligible real record must produce a replay record');
...
const report = history.replayHistory({ mechanism_runs: runs, routing_decisions: decisions, outcomes: outcomes, routing_actuals: [], transitions: [], usage_records: usage }, { min_passes: 3, top_k: 10, context_budget: { context_budget_escape_count: 0 } });
must(report.policy_reports.some(function (item) { return item.policy === 'skill-required' && item.tightening_rejection_count === 1; }), 'usage records must expose the skill-required policy cost');
must(report.metrics && report.metrics.replay_escape_count === 0 && report.metrics.tightening_rejection_count === 4, 'real replay must report tightening rejections without escapes');
must(report.metrics.boundary_hit_rate === 1 && report.metrics.unwanted_agent_wakeups === 4, 'boundary-aware metrics must follow the selected boundary mismatches');
