...
function runRecord(id, caseId, mechanismId, finishedAt, overrides) { return Object.assign({ schema_version: 'autoarmory/mechanism-run/v1', id: id, mechanism_id: mechanismId, case_id: caseId, actor: 'fixer', verified_by: 'independent-verifier', verified: true, result: 'pass', evidence: ['replay output'], counterexample: { kind: 'hash_mismatch', expected: 'match', observed: 'mismatch' }, input_sha256: hashA, output_sha256: hashB, environment_fingerprint: 'test-env', exit_code: 0, regression: false, started_at: finishedAt, finished_at: finishedAt }, overrides || {}); }
...
result = mechanism.recordMechanismRun(state, runRecord('run-bad-counterexample', 'case-evidence', 'mech-evidence', '2026-09-15T00:00:00.000Z', { counterexample: null }));
must(!result.ok && /counterexample/i.test(result.errors.join(' ')), 'missing counterexample must fail');
