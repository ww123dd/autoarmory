...
write(path.join(state, 'decision-scan', 'ready-for-verifier.jsonl'), [
  JSON.stringify({ schema_version: 'autoarmory/decision-draft/v1', change_id: 'registered-change', expected_transition: 'FAIL->PASS', owner: 'team-a', disposition: 'ready_for_verifier', verifier_candidate: { kind: 'registered', ref: 'fixture-verifier' }, session_id: 's1', turn_id: 't1', source_message_id: 'm1' }),
  JSON.stringify({ schema_version: 'autoarmory/decision-draft/v1', change_id: 'project-test-change', expected_transition: 'TEST->PASS', owner: 'team-a', disposition: 'ready_for_verifier', verifier_candidate: { kind: 'project_test', ref: 'node tests/run.js' }, changed_files: ['src/a.js'], commands: ['node tests/run.js'], session_id: 's2' })
].join('\n') + '\n');
...
must(registered.verifier_id === 'fixture-verifier' && registered.session_id === 's1' && registered.source_message_id === 'm1', 'registered pending job must preserve verifier and provenance');
must(project.verifier_candidate.kind === 'project_test' && project.mechanical_binding.kind === 'project_test' && project.expected_transition === 'TEST->PASS', 'mechanical pending job must carry pinned binding and transition');
