...
const { MODES, actionForMode, severityForMode } = require('./failure-modes');
...
for (const mode of MODES) ACTION_MAP[mode.mode] = mode.action;
...
function propose(incidents) {
  return incidents.map(function (incident) {
    const action = ACTION_MAP[incident.failure_mode] || actionForMode(incident.failure_mode) || 'collect_evidence';
    ...
    return {
      ...
      risk: RISK_MAP[incident.severity || severityForMode(incident.failure_mode)] || 'medium',
      status: 'candidate',
      change: {
