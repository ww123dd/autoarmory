...
function atomicWrite(file, text, options) {
  const opts = options || {};
  ...
  if (opts.backup !== false && fs.existsSync(file)) fs.copyFileSync(file, file + '.bak');
  fs.renameSync(temp, file);
}
function writeText(file, text, options) { atomicWrite(file, text, options); }
...
function writeJson(file, value, options) { writeText(file, JSON.stringify(value, null, 2) + '\n', options); }
function parseJsonl(text) { return normalizeText(text).split('\n').filter(Boolean).map(function (line) { return JSON.parse(line); }); }
function readJsonlResult(file) {
  if (!fs.existsSync(file)) return { exists: false, path: file, rows: [] };
  try { return { exists: true, path: file, rows: parseJsonl(readText(file)) }; }
  catch (error) {
    const backup = file + '.bak';
    if (fs.existsSync(backup)) return { exists: true, path: file, rows: parseJsonl(readText(backup)), recovered_from: backup };
    throw new Error('invalid JSONL in ' + file + ': ' + error.message);
  }
}
function readJsonlStrict(file) {
  if (!fs.existsSync(file)) throw new Error('JSONL file does not exist: ' + file);
  try { return parseJsonl(readText(file)); }
  catch (error) { throw new Error('invalid JSONL in ' + file + ': ' + error.message); }
}
function readJsonl(file) { return readJsonlResult(file).rows; }
function readJsonlDedup(file, keyFn) { const rows = readJsonl(file); const seen = {}; return rows.filter(function (row) { const raw = keyFn ? keyFn(row) : JSON.stringify(row); const key = raw === undefined || raw === null ? JSON.stringify(row) : raw; if (seen[key]) return false; seen[key] = true; return true; }); }
function writeJsonl(file, values, options) { writeText(file, values.map(function (value) { return JSON.stringify(value); }).join('\n') + (values.length ? '\n' : ''), options); }
function writeProjection(file, values, options) {
  const opts = options || {};
  const text = values.map(function (value) { return JSON.stringify(value); }).join('\n') + (values.length ? '\n' : '');
  const digest = crypto.createHash('sha256').update(text).digest('hex');
  const snapshots = file + '.snapshots';
  fs.mkdirSync(snapshots, { recursive: true });
  const snapshot = path.join(snapshots, digest + '.jsonl');
  if (!fs.existsSync(snapshot)) atomicWrite(snapshot, text, { backup: false });
  const link = file + '.tmp-link-' + process.pid + '-' + Date.now();
  try { fs.linkSync(snapshot, link); }
  catch (_) { fs.copyFileSync(snapshot, link); }
  if (fs.existsSync(file)) fs.unlinkSync(file);
  fs.renameSync(link, file);
  writeJson(file + '.latest.json', { schema_version: 'autoarmory/projection-pointer/v1', snapshot: snapshot, snapshot_sha256: digest, rows: values.length, updated_at: new Date().toISOString() }, { backup: true });
  if (opts.retain !== undefined) {
    const retain = Number(opts.retain);
    const files = fs.readdirSync(snapshots).filter(function (name) { return /\.jsonl$/.test(name); }).map(function (name) { const full = path.join(snapshots, name); return { file: full, mtimeMs: fs.statSync(full).mtimeMs }; }).sort(function (a, b) { return b.mtimeMs - a.mtimeMs; });
    for (const item of files.slice(retain)) { try { fs.unlinkSync(item.file); } catch (_) {} }
  }
  return { snapshot: snapshot, sha256: digest, rows: values.length };
}
...
module.exports = { readText, writeText, readJson, writeJson, readJsonl, readJsonlDedup, readJsonlResult, readJsonlStrict, writeJsonl, writeProjection, appendJsonl, dedupeJsonl, pruneBackups, printJson, sha256, parseArgs, redact, walkFiles };