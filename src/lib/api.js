...
function send(res, status, value, type) {
  const body = type === 'html' ? String(value) : JSON.stringify(value);
  res.writeHead(status, { 'content-type': type === 'html' ? 'text/html; charset=utf-8' : 'application/json; charset=utf-8', 'cache-control': 'no-store' });
  res.end(body);
}
...
function authorized(req, token) { return req.headers.authorization === 'Bearer ' + token; }
...
function startServer(options) {
  ...
  const token = opts.token || (allowWrite ? crypto.randomBytes(24).toString('hex') : '');
  return new Promise(function (resolve, reject) {
    const server = http.createServer(async function (req, res) {
      try {
        ...
        function writeGuard() {
          if (!allowWrite) { send(res, 403, { ok: false, error: 'write endpoints are disabled; start with --allow-write' }); return false; }
          if (!authorized(req, token)) { send(res, 401, { ok: false, error: 'missing or invalid bearer token' }); return false; }
          return true;
        }
        if (req.method === 'GET' && url.pathname === '/health') return send(res, 200, { ok: true, service: 'autoarmory', version: pkg.version, write_enabled: allowWrite });
        ...
        if (req.method === 'POST' && url.pathname === '/route') { if (!writeGuard()) return; const data = await body(req); const routeOptions = data.seed === undefined ? {} : { seed: Number(data.seed) }; const result = capability.route(capability.readCapabilities(registry), data.request, routeOptions); if (result.ok) { const rows = fs.existsSync(decisions) ? readJsonl(decisions) : []; rows.push(result); writeJsonl(decisions, rows); } return send(res, result.ok ? 200 : 409, result); }
        if (req.method === 'POST' && url.pathname === '/admit') { if (!writeGuard()) return; const data = await body(req); const result = admission.admit(Array.isArray(data) ? data : (data.candidates || [])); if (fs.existsSync(state)) { const rows = fs.existsSync(admissions) ? readJsonl(admissions) : []; rows.push.apply(rows, result.decisions); writeJsonl(admissions, rows); } return send(res, 200, result); }
        if (req.method === 'POST' && url.pathname === '/outcome') { if (!writeGuard()) return; const data = await body(req); const result = capability.recordOutcome(registry, outcomes, data.outcome || data, { executionTracesFile: path.join(state, 'execution-traces.jsonl') }); return send(res, result.ok ? 200 : 400, result); }
    ...
    server.listen(Number(opts.port || 0), opts.host || '127.0.0.1', function () { server.autoarmoryToken = token; server.autoarmoryAllowWrite = allowWrite; resolve(server); });
  });
}
