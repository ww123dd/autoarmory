  ...
  return new Promise(function (resolve, reject) {
    const server = http.createServer(async function (req, res) {
      try {
        ...
        if (req.method === 'POST' && url.pathname === '/outcome') { if (!writeGuard()) return; const data = await body(req); const result = capability.recordOutcome(registry, outcomes, data.outcome || data); return send(res, result.ok ? 200 : 400, result); }
