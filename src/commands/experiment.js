...
module.exports = function run(argv) {
  ...
  if (sub === 'compare') {
    ...
    if (!before || !after) { process.stderr.write('Usage: selfforge experiment compare <before.json> <after.json>\n'); return 2; }
  ...
  if (sub === 'shadow' || sub === 'canary') {
    const file = args._[1];
    if (!file) { process.stderr.write('Usage: selfforge experiment <shadow|canary> <candidate.json> [--percent 10]\n'); return 2; }
  ...
  process.stderr.write('Usage: selfforge experiment <compare|shadow|canary> ...\n'); return 2;
};
