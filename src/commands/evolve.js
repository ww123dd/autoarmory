...
const fs = require('fs');
const { parseArgs, writeJson, writeJsonl, printJson } = require('../lib/util');
...
const { readJsonl } = require('../lib/util');
module.exports = function run(argv) {
  ...
  const gated = candidates.map(function (candidate) { const result = gate(candidate); return Object.assign({}, candidate, { gate: result, status: result.ok ? 'gated' : 'rejected' }); });
  ...
  const result = { schema_version: 'selfforge/evolution/v1', input: path.resolve(input), incidents: incidents.length, candidates: gated.length, gated: gated.filter(function (item) { return item.status === 'gated'; }).length, rejected: gated.filter(function (item) { return item.status === 'rejected'; }).length, next: gated.slice(0, 5).map(function (item) { return { id: item.id, action: item.action, score: item.acquisition_score, risk: item.risk }; }) };
  if (args.json) printJson(result); else { process.stdout.write('SelfForge evolve\n'); process.stdout.write('  incidents=' + result.incidents + ' candidates=' + result.candidates + ' gated=' + result.gated + ' rejected=' + result.rejected + '\n'); for (const item of result.next) process.stdout.write('  ' + item.id + '  ' + item.action + '  score=' + item.score.toFixed(2) + '  risk=' + item.risk + '\n'); }
